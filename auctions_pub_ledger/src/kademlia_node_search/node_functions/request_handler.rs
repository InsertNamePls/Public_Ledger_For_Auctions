...
use crate::kademlia_node_search::config::K;
use crate::kademlia_node_search::node::Node;
use crate::kademlia_node_search::node_functions::routing_table::RoutingTable;
use bytes::Bytes;
use colored::*;
use hex::ToHex;
...
impl RequestHandler {
    pub async fn handle_ping(
        ...
        if node.lock().await.crypto.validate_request(
            ...
            if !routing_table.contains(&requester_id) {
                ...
                println!(
                    "{}",
                    "Added requester's information to routing table.".green()
                );
    ...
    pub async fn handle_find_node(
        ...
        println!(
            "{}",
            format!(
                "Received find_node request from [{}]: target_id {}, requester_id {}",
                requester_address,
                target_id.encode_hex::<String>(),
                requester_id.encode_hex::<String>()
            )
            .blue()
        );
        ...
        while new_nodes_found {
            ...
            for node_info in closest_nodes.iter() {
                ...
                match node
                    ...
                    .await
                    .client
                    .send_find_node_request(find_node_request, client_addr.clone())
                    .await
                {
                    Ok(response) => {
                        let nodes_from_response = RoutingTable::from_proto_nodes(response.nodes);
                        for new_node in nodes_from_response {
                            if !closest_seen.contains(&new_node)
                                && new_node.id != node.lock().await.id
                            {
        ...
        let proto_nodes = all_discovered_nodes
            .iter()
            .map(|node_info| ProtoNodeInfo {
                id: node_info.id.clone().to_vec(),
                address: node_info.addr.to_string(),
        ...
        while new_nodes_found {
            ...
            for node_info in closest_nodes.iter() {
                ...
                match node
                    ...
                    .await
                    .client
                    .send_find_value_request(find_value_request, client_addr.clone())
                    .await
                {
                    Ok(response) => {
                        ...
                        let nodes_from_response = RoutingTable::from_proto_nodes(response.nodes);
                        for new_node in nodes_from_response {
                            if !all_discovered_nodes.contains(&new_node)
                                && new_node.id != node.lock().await.id
                            {
                                all_discovered_nodes.push(new_node.clone());
                                new_nodes_found = true;
                    ...
                    Err(e) => {
                        eprintln!(
                            "Failed to send find_value request to {}: {}",
                            client_addr, e
                        );
                    }
                }
